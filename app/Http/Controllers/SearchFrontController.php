...
class SearchFrontController extends Controller
{
    ...
    {
        \Log::debug(json_encode($request->latitude));
        \Log::debug(json_encode($request->longitude));
    ...
    private static function getFormatedHotelList(array $hotels)
    {
        ...
        foreach ($hotels as $hotel) {
            //array_push($hotelListFormated, "Hotel {$hotel->getName()}, KM {$hotel->getFormatedDistance()}, EUR {$hotel->getPrice()}, Latitude {$hotel->getLatitude()}, Longitude {$hotel->getLongitude()}");

            $hotelForeach = [
                "Hotel" => $hotel->getName(),
                "KM" => $hotel->getFormatedDistance(),
                "EUR" => $hotel->getPrice(),
                "Latitude" => $hotel->getLatitude(),
                "Longitude" => $hotel->getLongitude()
            ];
            array_push($hotelListFormated, $hotelForeach,);
        }
        return $hotelListFormated;
    }
